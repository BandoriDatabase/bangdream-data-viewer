<template>
  <q-modal ref="modal" :content-css="{maxWidth: '800px', padding: '10px'}">
    <h4 class="uppercase">{{$t('title')}} <small>0.4.9</small></h4>
    <p>{{$t('subtitle')}}</p>
    <h5 class="uppercase">{{$t('features')}}</h5>
    <ul>
      <li>{{$t('feature-detail[0]')}}</li>
      <li>{{$t('feature-detail[1]')}}</li>
    ...
    <ul>
      ...
      <li>{{$t('bug-detail[3]')}}</li>
    </ul>
    <p v-html="$t('welcome-bug-report')"></p>
    <div class="action-buttons">
      <q-btn color="pink" @click="$refs.modal.close()">Close</q-btn>
    </div>
  </q-modal>
</template>
...
<i18n>
{
  "en": {
    ...
    "feature-detail": [
      "A button is added to sidebar so that you can view this Update Note again",
      "Slide note is drawn in Beatmap Viewer",
      "Add difficulty info on Music List page and on Beatmap Viewer page",
      "Sorting by music difficulty is supported",
      "Language won't change until you click the CONFIRM button"
    ],
    "bugs": "Fixed bugs",
    "bug-detail": [
      "Fix: No scrollbar visible if you click/touch card thumb and jump to card detail page",
      "Fix: Multiple BGM played while playing some beatmaps",
      "Fix: Missing translation text for Story Viewer page",
      "Fix: TW event detail display error"
    ],
  ...
  "zh-cn": {
    ...
    "feature-detail": [
      ...
      "音乐相关页面均加上了歌曲难度信息",
      "支持按照难度排序歌曲",
      "在点击确认按钮前不会保存对界面语言的修改"
    ],
    "bugs": "修复的问题",
    "bug-detail": [
      "修正了在抽卡详情里点击卡片头像跳转至详情后滚动条消失的问题",
      "修正了某些谱面会出现重复播放背景音乐的问题",
      "修正了故事查看页翻译文本的缺失",
      "修正了台服活动详情显示上的错误"
    ],
  ...
  "zh-tw": {
    ...
    "feature-detail": [
      ...
      "音樂相關頁面均加上了歌曲難度信息",
      "支持按照難度排序歌曲",
      "在點擊確認按鈕前不會保存對界面語言的修改"
    ],
    "bugs": "修復的問題",
    "bug-detail": [
      "修正了在抽卡詳情裏點擊卡片頭像跳轉至詳情後滾動條消失的問題",
      "修正了某些譜面會出現重復播放背景音樂的問題",
      "修正了故事查看頁翻譯文本的缺失",
      "修正了臺服活動詳情顯示上的錯誤"
    ],
