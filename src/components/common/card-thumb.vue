<template>
  <lazy-component @show="loadData">
    <div
      ...
      @click="() => {
        $emit('close');
        $router.push({ name: 'cardDetail', params: { server, situationId: (card || cardMap[server][situationId]).situationId } });
      }"
    >
      <my-q-img
        class="thumb-table"
        :src="`/assets/${$specialCardList[server].indexOf(card ? card.situationId : situationId) !== -1 ? server : 'jp'}/thumb/chara/card${cardGroup}_rip/${(card || cardMap[server][situationId]).resourceSetName}_${trained ? 'after_training' : 'normal'}.webp`"
      />
      ...
      <div
        v-for="i in (card || cardMap[server][situationId]).rarity"
        :key="i"
        :class="`thumb-rarity-${Number(trained)}-${i}`"
      ></div>
    </div>
    <div
      class="card-img-mini-parent"
      v-else-if="isReady"
      @click="() => {
        $emit('close');
    ...
    >
      ...
      <div
        v-for="i in (card || cardMap[server][situationId]).rarity"
        :key="i"
        :class="`thumb-mini-rarity-${Number(trained)}-${i}`"
      ></div>
    </div>
    <div
      class="card-img-parent"
