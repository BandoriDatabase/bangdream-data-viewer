<template>
  <div class="text-center" v-if="isReady">
    ...
    :src="`/assets/thumb/common_rip/practiceTicket${data.rewardId}.png`"></q-img>
    <q-img contain v-else-if="data.rewardType === 'practice_ticket'" class="reward-img"
    :src="`/assets/thumb/common_rip/skillticket_${data.rewardId}.png`"></q-img>
    <q-img contain v-else-if="data.rewardType === 'item'" class="reward-img"
    :src="`/assets/thumb/material_rip/material${String(data.rewardId).padStart(3, '0')}.png`"></q-img>
    <q-img contain v-else-if="data.rewardType === 'stamp'" class="reward-img"
    :src="`/assets-${server}/stamp/01_rip/${stampMap[server][data.rewardId].imageName}.png`"></q-img>
    <card-thumb v-else-if="data.rewardType === 'situation'" mini
    :cardId="Number(data.rewardId)" :server="server"></card-thumb>
    <q-img contain v-else class="reward-img"
    :src="`/assets/thumb/common_rip/${data.rewardType}.png`"></q-img>
