    ...
    <q-dialog v-model="isFilterVisible">
      <q-card>
        <q-card-section>
          <div class="row">
            ...
            <q-checkbox
              color="pink"
              class="col-6"
              v-model="selectBandId"
              v-for="opt in bandOption"
              :key="`band-${opt.value}`"
              :val="opt.value"
              :label="opt.label"
            ></q-checkbox>
          ...
          <div class="q-mt-md">
            ...
            <q-toggle
              v-model="selectMV"
              :label="$t('music.has-mv')"
            ></q-toggle>
            <q-toggle
              v-model="selectSP"
              :label="$t('music.has-sp')"
            ></q-toggle>
          </div>
        ...
        <q-card-actions>
          ...
          <q-btn
            color="pink"
            @click="doFilter(server), saveFilter(), isFilterVisible = false"
          >
            {{$t('common.apply-save')}}
          </q-btn>
        </q-card-actions>
    ...
    >
      <div class="row">
        <div
          ...
          :key="`music-${music.musicId}`"
        >
          <div
            class="row music cursor-pointer"
            @click="$router.push(`/music/${server}/${music.musicId}`)"
          >
            <q-card
              class="music-cover col-md-6 col-4"
              v-lazy:background-image="music.jacket"
            >
              <q-badge
                v-if="music.hasMV"
                align="top"
                style="font-size: 16px;"
              >{{$t('music.has-mv')}}</q-badge>
            </q-card>
            <div class="music-desc col-md-6 col-8">
              ...
              <div class="music-levels">
                ...
                <span
                  v-if="music.difficulty[4]"
                  class="music-level music-level-special"
                >{{music.difficulty[4]}}</span>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async updateData (server) {
      ...
      document.title = `${this.$t('music.title', { srv: this.$t(`common.${this.server}`) })} | Bandori Top`
      this.isReady = true
    },
