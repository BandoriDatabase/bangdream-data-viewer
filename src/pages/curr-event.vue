    ...
    <div class="row q-col-gutter-sm" v-if="isEventReady">
      <!-- <q-img>
        <div v-if="currentEvent[server].eventType === 'challenge' || currentEvent[server].eventType === 'versus' || currentEvent[server].eventType === 'live_try' || currentEvent[server].eventType === 'mission_live'" class="event-cover"
          :style="{ 'background-image': `url(/assets/event/${currentEvent[server].assetBundleName}/topscreen_rip/trim_eventtop.webp), url(/assets/event/${currentEvent[server].assetBundleName}/topscreen_rip/bg_eventtop.webp)` }"></div>
        <div v-else class="event-cover" :style="{ 'background-image': `url(/assets/event/${currentEvent[server].assetBundleName}/topscreen_rip/bg_eventtop.webp)` }"></div>
      ...
      <div class="col-12 col-md-6">
        <q-card>
          ...
          <q-card-section class="column items-center">
            <q-img contain class="event-banner" v-if="server != 'en'" :src="`/assets/${server}/homebanner_rip/banner_event${padEventId(currentEvent[server].eventId)}${currentEvent[server].eventId > 13 ? '' : '_open'}.webp`"/>
            <q-img contain class="event-banner" v-else-if="server === 'en' && currentEvent[server].eventId >= 3" :src="`/assets/${server}/homebanner_rip/banner_event${padEventId(currentEvent[server].eventId)}${currentEvent[server].eventId > 13 ? '' : '_open'}.webp`" />
            <q-img contain class="event-banner" v-else-if="server === 'en'" :src="`/assets/${server}/homebanner_rip/banner-0${14 + currentEvent[server].eventId * 2}.webp`" />
            ...
            <div class="row q-col-gutter-sm">
              <div class="col-6"><card-thumb :situationId="Number(eventNormalCardId)" :server="server"></card-thumb></div>
              <div class="col-6"><card-thumb :situationId="Number(eventSpecialCardId)" :server="server"></card-thumb></div>
            </div>
            <p class="q-mt-md text-h5">{{$t('event.reward-stamp')}}</p>
            <q-img v-if="eventRewardStamp" style="max-width: 220px;" :src="`/assets/${server}/stamp/01_rip/${eventRewardStamp.imageName}.webp`" />
            <q-spinner-facebook v-else color="pink" size="48px"></q-spinner-facebook> -->
            <p class="q-mt-md text-h5">{{$t('event.bonus-attr-card')}}</p>
            <!-- <img v-if="isDegreeReady" class="responsive" style="max-width: 100%;" v-lazy="`/assets/${server}/event/${currentEvent[server].assetBundleName}/images_rip/event_point_banner.webp`"> -->
            <div v-if="isDegreeReady">
              ...
              <div v-for="item in currentEvent[server].detail.characters" :key="item.characterId" class="row items-center justify-center q-my-sm">
                <img :src="`chara_icon_${item.characterId}.png`" style="width: 64px; height: 64px;" class="responsive">
            ...
            <q-img v-if="isBadgeReady" class="badge" :src="`/assets/thumb/common_rip/${eventItemMap[server][currentEvent[server].eventId].badgeAssetBundleName}.webp`" />
            <q-spinner-facebook v-else color="pink" size="48px"></q-spinner-facebook>
            ...
            <div v-if="isDegreeReady && (currentEvent[server].eventId >= 13 || server === 'cn')" class="event-degree relative-position">
              <q-img :src="`/assets/${server}/thumb/degree_rip/${degreeMap[server][currentEvent[server].rankingRewards[0].rewardId].imageName}.webp`" class="absolute-left"></q-img>
              <q-img :src="`/assets/${server}/thumb/degree_rip/event_point_1.webp`" class="absolute-left"></q-img>
              <q-img style="height: 50px; width: 50px" :src="`/assets/${server}/thumb/degree_rip/event_point_icon_1.webp`" class="absolute-left"></q-img>
            </div>
            <div v-else-if="isDegreeReady" class="event-degree" :style="{ 'background-image': `url(/assets/${server}/thumb/degree_rip/${degreeMap[server][currentEvent[server].rankingRewards[0].rewardId].imageName}.webp)` }"></div>
            <!-- <div v-else-if="isDegreeReady && server === 'tw'" class="event-degree" :style="{ 'background-image': `url(/assets/tw/thumb/degree_event_point_icon_1.webp), url(/assets/tw/thumb/degree_event_point_1.webp), url(/assets/tw/thumb/degree_${degreeMap[server][currentEvent[server].rankingRewards[0].rewardId].imageName}.webp)` }" /> -->
            <q-spinner-facebook v-else color="pink" size="48px"></q-spinner-facebook>
            <span v-if="currentEvent[server].eventType === 'challenge'">
              <div class="row justify-center" v-if="isDegreeReady">
                <div class="event-degree relative-position" v-for="eventMusic in currentEvent[server].detail.musics" :key="eventMusic.seq">
                  <q-img :src="`/assets/${server}/thumb/degree_rip/${degreeMap[server][eventMusic.musicRankingRewards[0].resourceId].imageName}.webp`" class="absolute-left"></q-img>
                  <q-img :src="`/assets/${server}/thumb/degree_rip/score_ranking_1.webp`" class="absolute-left"></q-img>
                </div>
              </div>
              <q-spinner-facebook v-else color="pink" size="48px"></q-spinner-facebook>
            </span>
            <span v-if="currentEvent[server].eventType === 'live_try'">
              <div class="row justify-center" v-if="isDegreeReady">
                <div class="event-degree relative-position">
                  <q-img :src="`/assets/${server}/thumb/degree_rip/${degreeMap[server][Object.values(this.currentEvent[server].detail.liveTryLevelRewardDifficultyMap.entries.normal.entries).find(e => e.resourceType === 'degree').resourceId].imageName}.webp`" class="absolute-left"></q-img>
                  <q-img :src="`/assets/${server}/thumb/degree_rip/try_clear_normal.webp`" class="absolute-left"></q-img>
                </div>
            ...
            <span class="column items-center q-mt-md" v-if="currentEvent[server].eventType === 'challenge'">
              ...
              <div class="row justify-center">
                ...
                <q-img :src="`/assets/musicjacket/${musicMap[server][eventMusic.musicId].jacketImage}_rip/thumb.webp`"
          ...
          <q-card-section>
            ...
            <div style="max-height: 800px; width: 100%; overflow: auto;">
              ...
              class="row items-center">
                <q-img :src="`/assets/story/bg/event/eventstory${item.coverImage}_rip/EventStoryScreenImage${item.coverImage}.webp`"
  ...
  computed: {
    ...
    eventBGM () {
      ...
      return {
        ...
        pic: `/assets/event/${assetBundleName}/images_rip/logo.webp`
      }
    },
    pointRewards () {
