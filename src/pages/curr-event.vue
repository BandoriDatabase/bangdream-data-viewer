<template>
  <q-page padding>
    ...
    >
      ...
      >
        <img
          :src="`/assets/jp/event/${currentEvent[server].assetBundleName}/topscreen_rip/trim_eventtop.webp`"
          alt="Event Character Trim"
          v-show="false"
        >
        <img
          :src="`/assets/jp/event/${currentEvent[server].assetBundleName}/topscreen_rip/bg_eventtop.webp`"
          alt="Event Background"
          v-show="false"
        >
        <img
          :src="`/assets/jp/event/${currentEvent[server].assetBundleName}/images_rip/logo.webp`"
          alt="Event Logo"
          v-show="false"
        >
        <my-q-img
          ...
          @click="$refs.eventtop.$viewer.show()"
        >
          <div
            v-if="currentEvent[server].eventType === 'challenge' || currentEvent[server].eventType === 'versus' || currentEvent[server].eventType === 'live_try' || currentEvent[server].eventType === 'mission_live'"
            class="event-cover"
            :style="{
          ...
          <div
            v-else
            class="event-cover"
            :style="{ 'background-image': `url(/assets/jp/event/${currentEvent[server].assetBundleName}/topscreen_rip/bg_eventtop.webp)` }"
          ></div>
        ...
        <q-card>
          ...
          <q-card-section class="column items-center">
            ...
            <my-q-img
              contain
              class="event-banner"
              v-else-if="server === 'en' && currentEvent[server].eventId >= 3"
              :src="`/assets/${server}/homebanner_rip/banner_event${padEventId(currentEvent[server].eventId)}${currentEvent[server].eventId > 13 ? '' : '_open'}.webp`"
            />
            <my-q-img
              contain
            ...
            ></a-player>
            <div
              class="full-width column items-center q-mt-md"
            ...
            <div
              class="full-width column items-center q-mt-md"
              v-else-if="Number(currentEvent[server].endAt) > Date.now()"
            >
            ...
            <div
              class="full-width column items-center q-mt-md"
              v-else-if="Number(currentEvent[server].endAt) < Date.now() && Number(currentEvent[server].distributionStartAt) > Date.now()"
            >
            ...
            <div
              class="full-width column items-center q-mt-md"
              v-else-if="Number(currentEvent[server].distributionStartAt) < Date.now() && Number(currentEvent[server].publicEndAt) > Date.now()"
            >
            ...
            <div class="row full-width justify-around items-center">
              ...
              <div class="col-6">
                <div class="row items-center justify-center q-my-sm">
                  <my-q-img
                    :src="`chara/chara_icon_${item.characterId}.png`"
                    style="width: 64px; height: 64px;"
                    v-for="item in currentEvent[server].detail.characters"
            ...
            <p
              ...
              v-if="currentEvent[server].detail.eventCharacterParameterBonus"
            >
              <span>{{$t('event.both-match')}}</span>
              <span
                v-if="currentEvent[server].detail.eventCharacterParameterBonus.performance"
                class="text-pink-6"
              >{{$t('common.perform')}} +{{currentEvent[server].detail.eventCharacterParameterBonus.performance}}%</span>
              <span
                v-if="currentEvent[server].detail.eventCharacterParameterBonus.technique"
                class="text-indigo-6"
              >{{$t('common.technic')}} +{{currentEvent[server].detail.eventCharacterParameterBonus.technique}}%</span>
              <span
                v-if="currentEvent[server].detail.eventCharacterParameterBonus.visual"
                class="text-orange-8"
              >{{$t('common.visual')}} +{{currentEvent[server].detail.eventCharacterParameterBonus.visual}}%</span>
            </p>
            ...
            <div class="row justify-around full-width">
              <div class="col-12 col-md-6 column items-center">
                ...
                <my-q-img
                  v-if="isBadgeReady"
                  class="badge"
                  :src="`/assets/jp/thumb/common_rip/${eventItemMap[server][currentEvent[server].eventId].resourceName}.webp`"
                >
              ...
              <div class="col-12 col-md-6 column justify-center items-center q-gutter-sm">
                ...
                >
                  <my-q-img
                    :src="`/assets/${server}/thumb/degree_rip/${degreeMap[server][currentEvent[server].rankingRewards[0].rewardId].baseImageName}.webp`"
                    class="absolute-left"
                  />
                  <my-q-img
                    :src="`/assets/${server}/thumb/degree_rip/event_point_1.webp`"
                    class="absolute-left"
                  />
                  <my-q-img
                    style="height: 50px; width: 50px"
                    :src="`/assets/${server}/thumb/degree_rip/event_point_icon_1.webp`"
                    class="absolute-left"
                  />
                  <q-tooltip>{{degreeMap[server][currentEvent[server].rankingRewards[0].rewardId].description}}</q-tooltip>
                </div>
                <div
                  v-else-if="isDegreeReady"
                  class="event-degree"
                  :style="{ 'background-image': `url(/assets/${server}/thumb/degree_rip/${degreeMap[server][currentEvent[server].rankingRewards[0].rewardId].baseImageName}.webp)` }"
                ></div>
                <q-spinner-facebook
                  v-else
                  size="48px"
            ...
            >
              ...
              <div class="row justify-center">
                ...
                >
                  <my-q-img
                    :src="`/assets/jp/musicjacket/musicjacket${Math.floor((eventMusic.musicId - 1) / 10 + 1) * 10}_rip/${musicMap[server][eventMusic.musicId].jacketImage}/thumb.webp`"
      ...
      <div class="col-12">
        <q-card>
          ...
          <q-card-section>
            ...
            <div
              v-for="rankingItem in rankingRewards"
              :key="rankingItem.fromRank"
              class="row"
            >
              ...
              <div class="col-9 row">
                <div
                  v-for="item in rankingItem.rewards"
                  :key="item.id"
          ...
          <q-card-section>
            ...
            >
              <q-tab
                :name="`tab-${item.scenarioId}`"
                :label="item.caption"
                v-for="item in currentEvent[server].detail.stories"
                :key="`tab-${item.scenarioId}`"
              ></q-tab>
            </q-tabs>
            <q-separator />
            <q-tab-panels
            ...
            >
              <q-tab-panel
                :name="`tab-${item.scenarioId}`"
                v-for="item in currentEvent[server].detail.stories"
                :key="`tabpanel-${item.scenarioId}`"
              >
                <my-q-img
                  :src="`/assets/jp/story/bg/event/eventstory${item.coverImage}_rip/eventstoryscreenimage${item.coverImage}.webp`"
                  :img-style="{ 'background-size': '100% calc(100% / 16 * 9)' }"
                />
  ...
  computed: {
    ...
    eventBGM () {
      ...
      return {
        ...
        src: `/assets/jp/${bgmAssetBundleName}_rip/${bgmFileName}.mp3`,
